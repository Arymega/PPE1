...
RAWOP=pd.DataFrame(pd.read_excel('RAW/HE188C.xlsx'))
...
LDWater=np.array(RAWLD.loc[RAWLD['kmol/m3']=='Water']).flatten()
LDWater=LDWater[1:5]
...
TSIN=OP[0]
TSOUT=OP[2]
FS=OP[4]*LD((TSIN+TSOUT)/2,LDWater)
SHELL=UDRDL(TSIN,TSOUT,FS)

#TUBE
# TTIN=OP[1]
# TTOUT=OP[3]
# FT=OP[5]
# PT=OP[7]*1e4/101325+1
# TUBE=UDRDL(TTIN,TTOUT,FT)
TUBE=np.zeros(len(SHELL))
